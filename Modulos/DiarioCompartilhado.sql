...
---------------------------------------------------------
-- TABELA: MIDIA (POR URL OU CAMINHO)
---------------------------------------------------------
CREATE TABLE Midia (
    Id_Midia SERIAL PRIMARY KEY,
    Tipo VARCHAR(20) NOT NULL CHECK (Tipo IN ('foto', 'video', 'link')),
...
AS $$
BEGIN
    IF p_tipo NOT IN ('foto', 'video', 'link') THEN
        RAISE EXCEPTION 'Tipo de mídia inválido: %', p_tipo;
    END IF;

    RETURN QUERY
    INSERT INTO Midia (Tipo, Caminho, NomeArquivo, MimeType, Id_Diario, Id_Observacao, Id_Mensagem)
    VALUES (p_tipo, p_caminho, p_nomearquivo, p_mimetype, p_id_diario, p_id_observacao, p_id_mensagem)
    RETURNING Id_Midia, Tipo, Caminho, NomeArquivo, MimeType, DataUpload,
              Id_Diario, Id_Observacao, Id_Mensagem;
END;
$$ LANGUAGE plpgsql;

---------------------------------------------------------
-- LISTAR MIDIAS POR DIARIO
---------------------------------------------------------
...
BEGIN
    RETURN QUERY
    SELECT Id_Midia, Tipo, Caminho, NomeArquivo, MimeType, DataUpload
    FROM Midia
    WHERE Id_Diario = p_id_diario
