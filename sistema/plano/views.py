...
def criar_plano(request):
    s = CriarPlanoIn(data=request.data)
...
def vincular_familiar(request):
    s = VincularFamiliarIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
...
def adicionar_neurodivergencia(request):
    s = AdicionarNeuroIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
...
def adicionar_metodo(request):
    s = AdicionarMetodoIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
...
def assinar_plano_terapeuta(request):
    s = AssinarPlanoTerapeutaIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
...
def anexar_arquivo(request):
    s = AnexarArquivoPlanoIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    f = d["arquivo"]
    try:
        blob = f.read()
        with get_conn() as conn, conn.cursor() as cur:
            cur.execute(SQL_ANEXAR, (d["id_plano"],
                        d["nome_arquivo"], d["tipo_mime"], blob))
...
def assinar_plano_familiar(request):
    s = AssinarPlanoFamiliarIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
...
def desvincular_familiar(request):
    s = DesvincularFamiliarIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
...
def remover_neurodivergencia(request):
    s = RemoverNeuroIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
...
def remover_metodo(request):
    s = RemoverMetodoIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
...
def atualizar_mensagem(request):
    s = AtualizarMensagemIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
...
def atualizar_cronograma(request):
    s = AtualizarCronogramaIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
            cur.execute(SQL_ATU_CRONO,
                        (d["id_plano"], d["cronograma_atividades"]))
...
def atualizar_abordagem(request):
    s = AtualizarAbordagemIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
...
def atualizar_objetivos(request):
    s = AtualizarObjetivosIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
            cur.execute(SQL_ATU_OBJ, (d["id_plano"],
...
def atualizar_grau_neurodivergencia(request):
    s = AtualizarGrauNeuroIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
            cur.execute(
                SQL_ATU_GRAU, (d["id_plano"], d["grau_neurodivergencia"]))
...
def excluir_arquivo_plano(request):
    s = ExcluirArquivoPlanoIn(data=request.data)
    s.is_valid(raise_exception=True)
    d = s.validated_data
    try:
        with get_conn() as conn, conn.cursor() as cur:
